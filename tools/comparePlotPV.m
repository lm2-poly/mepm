...
if isnumeric(P_model) && isnumeric(v_model) && isnumeric(P_literature) && isnumeric(v_literature)
    ...
    if any(P_model)
        if nargin == 8
            loglog(v_model,P_model,'-s','LineWidth',1.5,'Color',red);
            hold on
            t_model = text(v_model,P_model,string(round(P_model,0)),'FontSize',12);
        else
            loglog(myPlot,v_model,P_model,'-s','LineWidth',1.5,'Color',red);
            hold(myPlot,'on')
    ...
    if any(P_literature)
        if nargin == 8
            loglog(v_literature,P_literature,'-s','LineWidth',1.5,'Color',blue);
            t_lit = text(v_literature,P_literature,string(round(P_literature,0)),'FontSize',12);
        else
            loglog(myPlot,v_literature,P_literature,'-s','LineWidth',1.5,'Color',blue);
